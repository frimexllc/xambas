...
- Deuda menor pendiente: validar Stripe contra la API real (necesita `stripe listen` con llaves del usuario).
...
## Deuda técnica menor cerrada (Sep 2026)
- **Down-scale de imágenes antes de Groq**: `app/core/images.py` (`downscale_for_vision`, usa Pillow) reduce cualquier foto a máx. 1600px de lado antes de mandarla al modelo de visión de `ai_quote` — los proveedores de visión cobran y limitan por tokens de imagen, que escalan con la resolución, y una foto de celular a 12+ MP no mejora la estimación. El original subido se sigue guardando sin tocar (esta función solo afecta el payload que ve el modelo, no lo que se persiste). Si la imagen ya es chica se deja igual; un PNG con transparencia se convierte a JPEG/RGB; bytes no legibles se mandan tal cual (nunca rompe el flujo). 4 tests nuevos en `tests/test_images.py`.
- **Checkout de Stripe con paleta cálida**: `<Elements>` (`apps/client/src/panels/RequestsPanel.jsx`) no traía `appearance`, así que el `PaymentElement` renderizaba con el tema azul/blanco por defecto de Stripe, desentonando del resto de la interfaz. `apps/client/src/lib/stripe.js` ahora expone `getStripeAppearance()`, que resuelve el tema efectivo (elegido a mano o del sistema, mismo mecanismo que `theme.js`) y le pasa las mismas variables (`colorPrimary` coral, tipografía del sistema, `theme: "night"` en oscuro).
- **Bug de entorno local descubierto y corregido**: al correr la suite completa de backend, 4 pruebas de `milestones` fallaban con 500 al subir evidencia (archivo real). Causa: `apps/api/.env` tenía `STORAGE_PROVIDER=emergent` (heredado del entorno de preview original en la nube), que intenta llamar a un proxy externo (`integrations.emergentagent.com`) inalcanzable desde la máquina Windows del usuario — cualquier subida de archivo real (milestones, ai-quote) fallaba silenciosamente con un 500 sin manejar. El CI ya usaba `STORAGE_PROVIDER=local` correctamente (por eso siempre pasaba en GitHub Actions); se igualó el `.env` local al mismo valor. `.env` no está trackeado en git, así que este cambio es solo de esta máquina — si se clona el repo en otra máquina para desarrollo local sin credenciales de Emergent/R2, hay que poner `STORAGE_PROVIDER=local` a mano.
- Verificado: suite completa de backend 85 passed, 5 skipped (los 5 requieren `XAMBAS_RUN_EXTERNAL_TESTS=1`, llamada real a Groq). `yarn build` OK en las 4 apps, 25/25 tests de cliente sin cambios.
