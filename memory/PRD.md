...
- ✅ **Auth por token en `/api/recurring/*` y `/api/ai-quote/*`** (Sep 2026): dependencia `get_current_user` (header `Authorization: Bearer <token>`, mismo token que emite `otp/verify`) en `app/modules/identity/dependencies.py`. El `client_id` se toma del token, nunca del body; los endpoints por-id validan propiedad (403 si la suscripción/cotización es de otro cliente). `/status` y `/ai-quote/files/*` quedan públicos a propósito (este último es URL-capacidad, como `/milestones/files/*`, porque las `<img>` del navegador no mandan header). Frontend `apps/client`: `lib/api.js` adjunta el Bearer y rehidrata el token al cargar.
  - ✅ **Ampliado a `billing` + `milestones`** (Sep 2026): `require_client` / `require_provider` (nuevo) en `dependencies.py`. `POST /billing/payments`, `confirm-completion`, `GET /billing/payments[/{id}]`, `connect/onboarding-link`, `connect/status`; y todo `/milestones/plans*` + submit/release. El actor sale del token; los GET por-id y las acciones validan propiedad (403). `/billing/status|tiers|provider`, `/*/status` y `/*/files/*` siguen públicos. Frontend `apps/provider` también adjunta el Bearer ahora (`lib/api.js` + `App.jsx`). Tests: `test_billing.py` (13) nuevo + `test_milestones.py` reescrito con sesiones autenticadas. Total backend: 53 passed.
  - Pendiente: `/api/matching/*` (aceptar match, service-requests), `/api/messaging/*` (chat), `/api/reputation/reviews`, `/api/provider/dashboard`.
